...
class Node:
    ...
    def handleFlood(self, address, instant, flood, table, my_instant, port):
        ...
        if(self.flood_nr != flood): 
            self.flood_nr = flood
            if(self.flood_nr > 1):
                self.aux_routing_tables[self.server].clear()
                self.aux_routing_tables[self.server] = dict(self.neighbours)
        ...
        if (self.aux_routing_tables[self.server][self.keys[address]][2] == -1 or 
        self.aux_routing_tables[self.server][self.keys[address]][2] > my_instant-instant):
            self.aux_routing_tables[self.server][self.keys[address]] = (address, 1, my_instant-instant, self.aux_routing_tables[self.server][self.keys[address]][3])
            changed = 1
        ...
        for entry in table:
            ...
            if (key in self.aux_routing_tables[self.server]): #entrada já existe na minha tabela
                if(int(values[2]) != -1): #se o vizinho que me enviou já sabe o tempo
                    if(key != self.host): #entrada não é referente a mim
                        if (key in self.ips and self.ips[key] == address): #entrada é do vizinho da qual estou a receber
                            if(self.aux_routing_tables[self.server][key][2] == -1 or self.aux_routing_tables[self.server][key][2] > my_instant-instant):
                                self.aux_routing_tables[self.server][key] = (self.ips[key], 1, my_instant-instant, 'no')

                        else: #entrada NÃO é do vizinho que estou a receber
                            if (self.aux_routing_tables[self.server][key][2] != -1): #entrada já foi preenchida com algum valor previamente

                                if((self.aux_routing_tables[self.server][key][2] > tempo) #valor que recebo para um certo destino é menor
                                or (self.aux_routing_tables[self.server][key][2] == tempo and int(values[1]) + 1 < self.aux_routing_tables[self.server][key][1])):
                                #valor que recebo é igual ao que já tenho mas tem menor número de saltos
                                    self.aux_routing_tables[self.server][key] = (address, int(values[1]) + 1, tempo, self.aux_routing_tables[self.server][key][3])
                                    changed = 1
                                ...
                                if (self.ips[key] != address): #quem está a preencher não é o vizinho
                                    self.aux_routing_tables[self.server][key] = (address, int(values[1]) + 1, tempo, self.aux_routing_tables[self.server][key][3]) #altera o tempo (em ms) para lá chegar
                                    changed = 1
                                else:
                                    self.aux_routing_tables[self.server][address] = (address, 1, my_instant-instant, 'no')
                                    changed = 1

            else: #entrada não existe
                if(int(values[2]) != -1): #se o vizinho que me enviou já sabe o tempo
                    self.aux_routing_tables[self.server][key] = (address, int(values[1]) + 1, tempo, 'no')
                    changed = 1

        if (changed): #só continuo o flood se a minha tabela tiver alterado
            vizinhos = [x for x in self.aux_routing_tables[self.server] if self.aux_routing_tables[self.server][x][1] == 1]

            str_to_send = ""
            list_to_send = []
            for neighbour in self.aux_routing_tables[self.server]:
                neighbour_str = neighbour + ":" + ','.join(map(str, self.aux_routing_tables[self.server][neighbour]))
            ...
            self.routing_tables[self.server] = dict(self.aux_routing_tables[self.server])
